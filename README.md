...
3. **Persistencia:** en instancias sin disco, `beta-sessions.json`, waitlist, suscripciones y registros de push se pierden al redeploy. Para producción, monta disco (p. ej. `/var/data`) y `COMFORT_DATA_DIR=/var/data` (el `render.yaml` Blueprint ya lo declara; si creas el servicio manualmente, replica esa configuración).
...
5. **Pagos:** cuando LS esté listo, configura variables LemonSqueezy, webhook a `https://TU_DOMINIO/api/webhooks/lemonsqueezy` y `COMFORT_PUBLIC_PURCHASE=true`. Para **pagadores por onboarding + testers gratis con login beta**, usa **`COMFORT_ACCESS_MODE=hybrid`** (detalle en `comfort-ledger-beta/README.md`).
